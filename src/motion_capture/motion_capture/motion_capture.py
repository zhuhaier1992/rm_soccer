...
from rm_control.utils import get_ori
    ...
    def py_data_func(self, pFrameOfMocapData, pUserData):
        if pFrameOfMocapData == None:  
            ...
            for iBody in range(frameData.nRigidBodies):
                ...
                self.poses.pose.append(Pose2D(x=body.x/1000, y=body.y/1000, theta=get_ori(body.qz, body.qw)))
            ...
            if len(self.tdq)>2 and len(self.pdq)>2:
                ...
                if dt!=0:
                    for i in range(no_robots+1):
                        ...
                        self.twists.twist.append(Pose2D(x=x/1000, y=y/1000, theta=z))
